...
namespace SCPSLTests.Extensions.Drivers
{
    /// <summary>
    /// Construct new instance of NUnit3BytesFrameworkDriver.
    /// </summary>
    /// <param name="testDomain">The application domain in which to create the FrameworkController</param>
    /// <param name="reference">An AssemblyName referring to the test framework.</param>
    internal class NUnit3BytesFrameworkDriver(AppDomain testDomain, AssemblyName reference) : IFrameworkDriver
    {
        ...
        public string Load(string testAssemblyPath, IDictionary<string, object> settings)
        {
            ...
            _assembly = testDomain.Load(File.ReadAllBytes(testAssemblyPath));

            var idPrefix = string.IsNullOrEmpty(ID) ? "" : ID + "-";
            _testAssemblyPath = testAssemblyPath;
            try
            {
                _frameworkController = CreateObject(CONTROLLER_TYPE, _assembly, idPrefix, settings);
            }
        ...
        private object CreateObject(string typeName, params object?[] args)
        {
            try
            {
                return testDomain.CreateInstanceAndUnwrap(
                    reference.FullName, typeName, false, 0, null, args, null, null);
            }
            catch (TargetInvocationException ex)
            {
                throw new NUnitEngineException("The NUnit 3 bytes driver encountered an error while executing reflected code.", ex.InnerException);
            }
        }
    }
}
